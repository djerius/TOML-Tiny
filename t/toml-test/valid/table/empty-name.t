...
open my $fh, '<', "./t/toml-test/valid/table/empty-name.toml" or die $!;
...
is($actual, $expected1, 'table/empty-name - from_toml') or do{
  diag 'TOML INPUT:';
  diag "$toml";
...
ok(!$error, 'table/empty-name - to_toml - no errors')
  or diag $error;

is($reparsed, $expected1, 'table/empty-name - to_toml') or do{
