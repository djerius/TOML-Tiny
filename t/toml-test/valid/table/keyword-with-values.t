...
open my $fh, '<', "./t/toml-test/valid/table/keyword-with-values.toml" or die $!;
...
is($actual, $expected1, 'table/keyword-with-values - from_toml') or do{
  diag 'TOML INPUT:';
  diag "$toml";
...
ok(!$error, 'table/keyword-with-values - to_toml - no errors')
  or diag $error;

is($reparsed, $expected1, 'table/keyword-with-values - to_toml') or do{
