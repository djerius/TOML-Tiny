...
open my $fh, '<', "./t/toml-test/valid/key/zero.toml" or die $!;
...
is($actual, $expected1, 'key/zero - from_toml') or do{
  diag 'TOML INPUT:';
  diag "$toml";
...
ok(!$error, 'key/zero - to_toml - no errors')
  or diag $error;

is($reparsed, $expected1, 'key/zero - to_toml') or do{
