...
open my $fh, '<', "./t/toml-test/valid/key/start.toml" or die $!;
...
is($actual, $expected1, 'key/start - from_toml') or do{
  diag 'TOML INPUT:';
  diag "$toml";
...
ok(!$error, 'key/start - to_toml - no errors')
  or diag $error;

is($reparsed, $expected1, 'key/start - to_toml') or do{
