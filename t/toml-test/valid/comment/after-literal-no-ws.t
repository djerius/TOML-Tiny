...
open my $fh, '<', "./t/toml-test/valid/comment/after-literal-no-ws.toml" or die $!;
...
is($actual, $expected1, 'comment/after-literal-no-ws - from_toml') or do{
  diag 'TOML INPUT:';
  diag "$toml";
...
ok(!$error, 'comment/after-literal-no-ws - to_toml - no errors')
  or diag $error;

is($reparsed, $expected1, 'comment/after-literal-no-ws - to_toml') or do{
